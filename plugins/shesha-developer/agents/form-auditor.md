...
- **quality** — the detailed grading rubric in `SKILL_ROOT/references/form-quality-rubric.md` (validationErrors, labels, dropdown sources, primary action, editMode per form type — with the edge-case detail needed to resolve ambiguous checks; the compact floor it grades against is `references/form-quality.md`).
