...
6. **Multi-screen — fan out, one agent per screen (MUST for 2+ screens).** The conductor dispatches comprehension (Step 2) and structure authoring (Step 4a) **in parallel, one agent per screen**, via `superpowers:dispatching-parallel-agents`; each dispatched agent carries Contract A and **returns markup only — never pushes, never styles**. The barriers stay serial: theme set once first, the single gated push path, `shesha-design-system` styles **centrally** to keep the look coherent, and the browser-bound placement + visual verify runs on one session. Comprehension verifies each screen's placement independently. Dispatch playbook + fan-out map: [orchestration.md](orchestration.md).
