...
- **Watch the proxies as you go, not just in hindsight:** the expected fan-out is **one agent per screen** (comprehension + structure authoring — see [orchestration.md](orchestration.md)), which is *not* waste. The waste signal is `agents_dispatched` exceeding **one per screen** (redundant re-dispatch, multiple authors on the same screen), you're about to open a >50KB seed, you've taken more than one screenshot per screen, or **you're about to trigger a 2nd rebuild/restart cycle** — stop and take the cheaper path (scan all prereqs and do ONE build, build inline, read the cheatsheet, assert via a11y snapshot).
