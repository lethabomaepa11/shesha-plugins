...
**Then run the mechanical guardrail validator (mandatory, blocking):** `node <skill-root>/scripts/validate-guardrails.js <path to your edited form>` — it catches the recurring render-killers (buttonGroup with no primary, standalone action buttons, Navigate with an empty target, PascalCase datatable columns, dataContext missing `entityType`/`sourceType`, missing `validationErrors`, FK columns bound to the raw `…Id` scalar). Zero `fail` findings before push; fix `warn` findings unless there's a stated reason not to.
