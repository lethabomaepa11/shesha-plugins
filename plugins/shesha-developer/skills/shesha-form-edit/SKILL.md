...
- **API namespace is `app` vs `Shesha` per service — never guess it.** `FormConfiguration` lives under `/api/services/Shesha/…`; `Metadata`, `Module`, `EntityConfig`, and `ReferenceList` live under `/api/services/app/…`. A 404 is usually the wrong namespace or an unresolved `fullClassName`, NOT a missing resource — re-check the namespace and retry before concluding something doesn't exist. Endpoint table: [api.md](references/api.md).
