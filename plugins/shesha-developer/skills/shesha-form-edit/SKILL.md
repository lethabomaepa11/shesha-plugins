...
**FIRST action of Step 4.5 (mandatory) — run `scripts/backend-probe.mjs` before ANY hand-rolled probing.** It collapses module id + entity resolve + metadata + reflist existence into a single call, so run it first and read its output instead of firing the separate round-trips below one at a time — see [api.md Combined one-shot backend probe](references/api.md). The manual resolution steps that follow are the fallback/explanation for when the probe leaves a gap, not the default path. **Never guess `GetByName`/`GetAll` routes — a 404 is usually the wrong `app` vs `Shesha` namespace, not an absent form (verify a KNOWN form resolves before concluding one is missing).**

**If the probe reveals missing prereqs (entity / reflist / endpoint / permission), do NOT fix them one-at-a-time.** Dispatch the `shesha-developer:fullstack-prereq-checker` agent ONCE to enumerate the FULL gap, plan every domain + app-layer change together, and apply them in a **single rebuild + (double-)boot** — the enforced scan-once-build-once path in [backend-restart.md](references/backend-restart.md). Serial discover→build→discover→build is the single biggest wall-clock sink (one run burned 5+ rebuild/boot cycles on it). Two facts that otherwise cost a rebuild-to-discover: a `[ReferenceList]` attribute **auto-creates its (empty) reflist on boot** (seed items, don't create a duplicate) and its DB column needs the **`Lkp` suffix** — see [modern-renderer-gotchas.md](references/modern-renderer-gotchas.md) §4 + `shesha-developer:domain-model`.
...
| Read-only-from-versionless, code-mode `hidden` visibility, anonymous public-form pattern, backend-bootstrap playbook | [references/modern-renderer-gotchas.md](references/modern-renderer-gotchas.md) |
