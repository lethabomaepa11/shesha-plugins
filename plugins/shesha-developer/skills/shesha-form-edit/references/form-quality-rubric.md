# Form quality rubric (detailed grading companion)

The exhaustive, edge-case-annotated companion to the compact floor in
[form-quality.md](form-quality.md). The floor lists the terse per-form construction
rules you apply on **every** build; this rubric carries the full rationale, the silent-failure
symptoms, the defect descriptions, and the project-specific worked example. Read it when a
floor check is **ambiguous**, when **auditing / grading** a form, or when you need the "why"
behind a floor bullet. Any Shesha form QA process (human reviewer or browser-driven
automation) grades against exactly these properties — the floor and this rubric are the same
contract at two levels of detail.
