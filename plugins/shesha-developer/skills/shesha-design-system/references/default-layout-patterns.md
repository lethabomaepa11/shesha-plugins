...
- **Everything on the far right of a table's header zone aligns flush with the table's right edge** — toolbar buttons (Refresh, column toggle, Add), alerts/notices, count labels ("1–50 of 5,221"), and pagination all share one right edge: the table's. Mechanics (browser-verified 2026-07-05): the toolbar-row container (the one carrying `className:"sha-index-toolbar"`) needs `justifyContent:"flex-end"` (or `"space-between"` with a left search/filter cluster) **plus its inner div forced to full width** — on 0.45 via `desktop.dimensions.width:"100%"`, on **0.43 via the `style` channel: `style: "return { width: '100%' };"`** (0.43's container has NO flat `width` field — only minWidth/maxWidth — and the `style` JS-string is the one channel that reaches the shrink-wrapped `sha-components-container-inner` div; without it the cluster renders left no matter what `justifyContent` says). `justifyContent` without an explicit `display:"flex"` is also inert. Nothing in the header zone overhangs or falls short of the table edge. The canonical table seeds carry this recipe.
